...
| 2. Tracking/ReID (GTATrack: Deep-EIoU + GTA-Link) | Asociación global de tracklets: grafo con splitter (DBSCAN) + connector (Hungarian) | T2 ya implementa Hungarian + apariencia (embeddings OSNet de BoT-SORT) + posición + equipo como restricción dura — pero **solo en la frontera entre chunks consecutivos** (`Trackers/cross_chunk_reid.py::ChunkBridge._match_pair`). No existe splitter ni reconciliación de tracklets fragmentados dentro de un mismo chunk. `BOTSORT_TRACK_BUFFER` estaba en 200 (~6.7s a 30fps), por encima del umbral que el research señala como zona de "deriva fantasma" (>150 frames) — **corregido 2026-07-03, ver ablation en T8 más abajo: bajado a 100 sin coste de calidad medido** | Extender T2 a asociación global — mayor impacto de las 5 áreas, esfuerzo incremental bajo porque reutiliza infraestructura ya construida |
...
**Hallazgo adicional (2026-07-03, al planificar la implementación):** el voto por mayoría de `Main.py:287-309` es hoy un **no-op literal**. `get_player_team` cachea el equipo de cada ID la primera vez que lo ve (`team_assigner.py:103-104`) y devuelve siempre el valor cacheado — así que todos los "votos" por frame de un mismo ID son idénticos y la mayoría no puede corregir nada. En la práctica, el equipo de cada jugador se decide en un único frame (el primero en que aparece su ID) y es irreversible durante todo el partido. La fase "Team voting" solo reescribe el mismo valor. `debug_player_detection.py` duplica esta misma lógica muerta (líneas 64-76).

**Opción A — Arreglar dentro del paradigma actual (HSV + K-Means), esfuerzo bajo. Diseño detallado (2026-07-03):**

- **T7.a — Extracción de color robusta** (`team_assigner.py::get_player_color`): mantener el recorte geométrico (mitad superior, 60% central) y añadir máscara HSV de césped — excluir píxeles en el rango de verde antes de promediar. Fallback al promedio sin máscara si sobreviven menos de un % mínimo de píxeles (jersey verde, mala luz). De paso, sustituir `KMeans(n_clusters=1, n_init=10)` por frame/crop por un promedio numpy directo — con K=1 el centroide ES la media, la versión actual paga 10 inicializaciones de K-Means para calcular una media. Esta ganancia de eficiencia no es cosmética: habilita T7.c/d, que necesitan re-extraer color muchas más veces que el cacheo actual. Config: `TEAM_HSV_MASK_ENABLED` (flag A/B), `TEAM_GRASS_HUE_LOW/HIGH`, `TEAM_GRASS_MIN_PIXELS_PCT`.
- **T7.b — Burn-in multi-frame para los centroides**: `assign_team_color` deja de entrenar solo con el frame 0 — muestrea colores de jugadores a lo largo de los primeros `TEAM_BURNIN_SEC` segundos (cada `TEAM_SAMPLE_EVERY_N_FRAMES`) y entrena el K-Means global con ese pool. Robustez contra un frame 0 malo (pocos jugadores, sombra dura).
- **T7.c — Re-cluster periódico con anclaje de identidad**: re-entrenar el K-Means cada `TEAM_RECLUSTER_SEC` sobre los colores de la ventana reciente, **anclando los labels**: los centroides nuevos se asignan a equipo 1/2 por cercanía a los centroides anteriores (matching 2x2), nunca por el label arbitrario que devuelva sklearn — sin esto, un re-fit puede intercambiar los equipos a mitad de partido. `team_flip_rate` (ya en perf_monitor) es el detector de regresión de esto.
- **T7.d — Voto deslizante real por ID**: eliminar el cacheo permanente de `get_player_team` (recalcular color por frame o cada N frames — ya barato gracias a T7.a) y sustituir el voto global muerto de `Main.py` por mayoría deslizante sobre los últimos `TEAM_VOTE_WINDOW_SEC` por ID. Extraer esta lógica a un helper compartido (en `TeamAssigner`) para que `Main.py` y `debug_player_detection.py` usen la misma implementación en vez de duplicarla. Nota de contrato: el JSON exporta un único `team` por ID — el exportado sigue siendo la mayoría global (sin cambio de schema); el equipo por-frame alimenta posesión y colores del vídeo anotado.
- **T7.e — Portero fuera del clustering**: `tracker.py` deja de descartar la clase — el portero sigue yendo a `tracks["players"]` (ningún consumidor downstream cambia) pero con flag `"is_goalkeeper": True` (leído con `.get()` para compatibilidad con stubs viejos). El team assigner lo **excluye del entrenamiento de centroides** (el fix crítico — su kit distinto deja de arrastrar los clusters) y le asigna equipo por cercanía de color con el voto deslizante, como a cualquier jugador. Asignación espacial (por portería defendida) se difiere: el cambio de lado al descanso la hace no-trivial, y el flag interno deja la puerta abierta. No se exporta al JSON (cambio de schema = PR conjunto con dashboard, fuera de alcance de T7).

**Validación T7-A (sin GPU propia, mismo patrón que el ablation del buffer):** `debug_player_detection.py` con `SKIP_VIDEO_OUTPUT=true` sobre los dos clips ya conocidos de `video_OG.mp4` (31-91s juego abierto, 1638-1698s córner) — comparar `team_flip_rate` (objetivo <2%) antes/después; el flag `TEAM_HSV_MASK_ENABLED` permite aislar el efecto de la máscara. Una corrida final con render activado para inspección visual (colores de equipo estables, portero bien asignado). Vigilar el tiempo de la fase "Team assignment" en perf_monitor — T7.d recalcula más a menudo; T7.a lo compensa.

- Esfuerzo: 1-2 días. Cero dependencias nuevas.
- Riesgos específicos: máscara de césped mal calibrada en canchas no-verdes (mitigado: flag + fallback), label flip en re-cluster (mitigado: anclaje + team_flip_rate), coste extra de recalcular por frame (mitigado: quitar el K-Means por crop lo compensa de sobra).
...
El research advierte explícitamente contra subir el buffer de persistencia del tracker online como forma de "aguantar" oclusiones largas (mantener una predicción activa >150 frames sin detección física genera "derivas fantasma": el tracker asigna identidades de jugadores activos a trayectorias lineales erráticas). Nuestro `BOTSORT_TRACK_BUFFER=200` estaba en esa zona de riesgo. En vez de esperar a T8 para tocarlo (como decía la primera versión de este plan), se validó de forma aislada y barata con `debug_player_detection.py` + `SKIP_VIDEO_OUTPUT=true` (sin renderizar, solo métricas) sobre `video_OG.mp4`, comparando `BOTSORT_TRACK_BUFFER` en {10, 100, 150, 200} con `best_jugadores_v2.pt`, en dos clips de 60s:

| Clip | Buffer 10 | Buffer 100 | Buffer 150 | Buffer 200 |
|---|---|---|---|---|
| Juego abierto (seg. 31-91) — `id_churn_ratio` | 3.77 ❌ | 2.84 | 2.84 (idéntico) | 2.84 (idéntico) |
| Córner, clip empieza justo antes del saque (seg. 1638-1698) — `id_churn_ratio` | 2.85 | 2.50 | 2.50 (idéntico) | 2.48 |

Lectura: el salto real está entre 10 y 100 (10 rompe el objetivo ≤3.0 en el clip de juego abierto). De 100 en adelante, el efecto se aplana — incluso en el córner, el clip elegido a propósito por tener la aglomeración de jugadores más dura del dataset de prueba, pasar de 100 a 200 (doblar el buffer) cambia el `id_churn_ratio` en -0.8% (2.50→2.48), dentro de ruido de medición. `BOTSORT_TRACK_BUFFER` no es una palanca de cómputo real en este pipeline (es bookkeeping de matching húngaro sobre ~20-25 tracks, no inferencia GPU), así que no hay ningún trade-off de coste que justifique quedarse en 200 cuando 100 da la misma calidad medida.

**Cambio aplicado:** `BOTSORT_TRACK_BUFFER` default bajado de 200 a 100 en `config.py`, `Trackers/botsort.yaml` y `.env.example`. Esto **desacopla el cambio de T8** — la cautela original de este plan ("no bajar el buffer sin la reconciliación offline de T8 como red de seguridad") ya no aplica a este rango concreto porque ahora hay evidencia directa, no solo teórica, de que 100 no cuesta calidad. T8 sigue teniendo valor propio (arregla fragmentación intra-chunk que ningún valor de buffer puede resolver del todo), pero deja de ser prerrequisito de este cambio.

**Límite honesto de esta validación:** son 2 minutos de un partido (2 clips de 60s, uno de juego abierto y uno de córner), no la validación completa de partido entero en Kaggle que T2 todavía tiene pendiente. No cubre el caso de una oclusión realmente larga (p. ej. un jugador tapado 5-8s en un poste-pelea de gol). Como es un cambio de una sola línea y trivialmente reversible, se adoptó igualmente — pero si la validación completa de T2/T8 en Kaggle revela una regresión, revertir a 200 es inmediato.
...
| Subir aún más el buffer de persistencia del tracker (Kalman) para tapar oclusiones largas | Genera derivas fantasma; la vía correcta es reconciliación offline (T8), no más buffer online. Ya bajamos de 200 a 100 (ver T8) sin coste de calidad medido — subir de nuevo iría en la dirección contraria a la evidencia | Research + ablation propio 2026-07-03 |
